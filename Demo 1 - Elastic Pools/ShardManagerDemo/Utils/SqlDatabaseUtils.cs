// Besed on https://code.msdn.microsoft.com/windowsapps/Elastic-Scale-with-Azure-a80d8dc6
...
namespace ShardManagerDemo.Utils
{
    /// <summary>
    ...
    internal static class SqlDatabaseUtils
    {
        ...
        public static bool TryConnectToSqlDatabase()
        {
            ...
            {
                Console.WriteLine("Failed to connect to SQL database with connection string:");
                Console.WriteLine("\n{0}\n", connectionString);
                Console.WriteLine("If this connection string is incorrect, please update the Sql Database settings in App.Config.\n\nException message: {0}", e.Message);
                return false;
            }
        }
        ...
        public static void CreateDatabase(string server, string db, string elasticPool)
        {
            Console.WriteLine("Creating database {0}", db);
            using (ReliableSqlConnection conn = new ReliableSqlConnection(
                Configuration.GetConnectionString(server, MasterDatabaseName),
                SqlRetryPolicy,
                SqlRetryPolicy))
            {
                ...
                if (engineEdition == 5)
                {
                    ...
                    SqlRetryPolicy.ExecuteAction(() =>
                        {
                            if (!DatabaseExists(server, db))
                            {
                                ...
                                cmd.CommandText = elasticPool == null 
                                    ? $"CREATE DATABASE {BracketEscapeName(db)} (EDITION = '{Configuration.DatabaseEdition}')" 
                                    : $"CREATE DATABASE {BracketEscapeName(db)} (EDITION = '{Configuration.DatabaseEdition}', SERVICE_OBJECTIVE = ELASTIC_POOL (name = {BracketEscapeName(elasticPool)} ))";
                                cmd.CommandTimeout = 60;
                                cmd.ExecuteNonQuery();
                            }
                        });
                    ...
                    {
                        Console.WriteLine("Waiting for database {0} to come online...", db);
                        Thread.Sleep(TimeSpan.FromSeconds(5));
                    }

                    Console.WriteLine("Database {0} is online", db);
                }
                else
                {
                    // Other edition of SQL DB
                    cmd.CommandText = $"CREATE DATABASE {BracketEscapeName(db)}";
        ...
        {
            Console.WriteLine("Dropping database {0}", db);
            using (ReliableSqlConnection conn = new ReliableSqlConnection(
                Configuration.GetConnectionString(server, MasterDatabaseName),
                SqlRetryPolicy,
                SqlRetryPolicy))
            {
                ...
                if (engineEdition == 5)
                {
                    ...
                    cmd.CommandText = $"DROP DATABASE {BracketEscapeName(db)}";
                    cmd.ExecuteNonQuery();
                }
                else
                {
                    cmd.CommandText = $@"ALTER DATABASE {BracketEscapeName(db)} SET SINGLE_USER WITH ROLLBACK IMMEDIATE DROP DATABASE {BracketEscapeName(db)}";
                    cmd.ExecuteNonQuery();
                }
            }
        }
        ...
        {
            Console.WriteLine("Executing script {0}", schemaFile);
            using (ReliableSqlConnection conn = new ReliableSqlConnection(
        ...
        private class ExtendedSqlDatabaseTransientErrorDetectionStrategy : ITransientErrorDetectionStrategy
        {
            ...
            /// </summary>
            private readonly SqlDatabaseTransientErrorDetectionStrategy _sqltransientErrorDetectionStrategy = new SqlDatabaseTransientErrorDetectionStrategy();
